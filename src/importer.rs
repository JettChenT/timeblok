use std::{rc::Rc, collections::HashSet, sync::Arc};

use chrono::NaiveDate;
use icalendar::{Calendar, Component, DatePerhapsTime};
use crate::{ir::{filter::{BDF, Filter}, Date, ExactDate}, environment::Environment, resolver::resolve_date};
...
impl Filter<Date> for SetFilter{
    fn check(&self, value: &Date, env: Option<&Environment>) -> bool {
        self.dates.contains(&resolve_date(value, env.unwrap()).unwrap())
    }
}
